 ...
 *
 * QUAN TRỌNG - Quy tắc hash của VNPay: 1. Sort params theo alphabet (key RAW)
 * 2. Hash data: key=URLEncode(value) - KEY KHÔNG ENCODE 3. Query string:
 * URLEncode(key)=URLEncode(value) - CẢ HAI ENCODE 4. Sử dụng HMAC-SHA512 5.
 * Charset: US-ASCII
 */
public class VNPayService {
     ...
     *
     * Code demo VNPay (vnpay_return.jsp): - Lấy tất cả params - Remove
     * vnp_SecureHash và vnp_SecureHashType - Gọi Config.hashAllFields(fields)
     * để tính hash - So sánh với vnp_SecureHash nhận được
     */
     ...
     *
     * QUAN TRỌNG: Trong callback, VNPay gửi params đã được URL encode Code demo
     * IPN: encode cả fieldName và fieldValue khi đọc từ request Code demo
     * Return: KHÔNG encode, dùng RAW values
     *
    ...
    public String getPaymentMessage(String responseCode) {
        switch (responseCode) {
            ...
            case "07":
                return "Tru tien thanh cong nhung giao dich bi nghi ngo";
            case "09":
                return "Chua dang ky InternetBanking";
            case "10":
                return "Xac thuc khong dung qua 3 lan";
            case "11":
            ...
            case "24":
                return "Huy giao dich";
            case "51":
                return "Khong du so du";
            case "65":
                return "Vuot han muc giao dich";
            case "75":
                return "Ngan hang bao tri";
            case "79":
                return "Sai mat khau qua nhieu lan";
            case "97":
                return "Chu ky khong hop le";
            case "99":
                return "Loi khong xac dinh";
            default:
                return "Loi: " + responseCode;
        }
    }
}
