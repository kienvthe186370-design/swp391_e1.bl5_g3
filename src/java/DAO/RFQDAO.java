...
public class RFQDAO extends DBContext {
    ...
    public boolean rejectProposedDate(int rfqID, String reason, int customerID) {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, RFQ.STATUS_DATE_REJECTED);
                ps.setString(2, reason);
                ps.setInt(3, rfqID);
                ps.executeUpdate();
            }
            
            insertHistory(conn, rfqID, oldStatus, RFQ.STATUS_DATE_REJECTED, "Date Rejected", 
                         "Khách hàng từ chối ngày giao mới. Lý do: " + reason, customerID, "customer");
    ...
    private String generateRFQCode(Connection conn) throws SQLException {
        ...
        String sql = "SELECT COUNT(*) + 1 as NextNum FROM RFQs WHERE YEAR(CreatedDate) = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, year);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
        ...
        return prefix + year + "-" + System.currentTimeMillis();
    }
