...
public class ProductDAO extends DBContext {
    ...
    /**
     * Calculate product status based on variant count and stock
     * @param variantCount - Number of active variants
     * @param totalStock - Total stock across all active variants
    ...
    /**
     * Lấy danh sách sản phẩm với filter, sort, pagination (backward compatible)
    ...
    /**
     * Lấy danh sách sản phẩm với filter, sort, pagination (with status filter)
     * Trả về List<Map> chứa thông tin sản phẩm + thông tin liên quan
     * 
     * @param search - Tìm kiếm theo tên sản phẩm (nullable)
     * @param categoryId - Filter theo category (nullable)
     * @param brandId - Filter theo brand (nullable)
     * @param isActive - Filter theo status (null = all, true = active, false = inactive)
     * @param statusFilter - Filter theo product status: "draft", "in_stock", "out_of_stock", or null for all
     * @param sortBy - Sắp xếp theo: "name", "price", "date", "stock"
     * @param sortOrder - "asc" hoặc "desc"
     * @param page - Trang hiện tại (bắt đầu từ 1)
     * @param pageSize - Số item mỗi trang
    ...
    public List<Map<String, Object>> getProducts(String search, Integer categoryId, Integer brandId,
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Calculate and add product status
    ...
    /**
     * Đếm tổng số sản phẩm (cho pagination) - backward compatible
     */
    ...
    /**
     * Đếm tổng số sản phẩm (cho pagination) - with status filter
     * @param statusFilter - Filter theo product status: "draft", "in_stock", "out_of_stock", or null for all
     */
    ...
    /**
     * Lấy danh sách sản phẩm với filter theo khoảng giá
     */
    public List<Map<String, Object>> getProductsWithPriceFilter(String search, Integer categoryId, Integer brandId,
        ...
        try {
            ...
            // Price filter - filter by MinPrice of variants
            if (minPrice != null) {
                sql.append("AND (SELECT MIN(SellingPrice) FROM ProductVariants WHERE ProductID = p.ProductID AND IsActive = 1) >= ? ");
            }
            if (maxPrice != null) {
                sql.append("AND (SELECT MIN(SellingPrice) FROM ProductVariants WHERE ProductID = p.ProductID AND IsActive = 1) <= ? ");
            }
    ...
    /**
     * Đếm tổng số sản phẩm với filter theo khoảng giá
     */
     ...
     * @return Map with keys: "draft", "in_stock", "out_of_stock"
     */
    public Map<String, Integer> getProductStatusCounts() {
        ...
        try {
            ...
            System.err.println("Error in getProductStatusCounts: " + e.getMessage());
            e.printStackTrace();
            // Return map with zeros on error
        } finally {
            try {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
    ...
    public boolean toggleProductStatus(int productId, boolean isActive) {
        ...
        try {
            conn = getConnection();
            String sql = "UPDATE Products SET IsActive = ?, UpdatedDate = GETDATE() WHERE ProductID = ?";
            ps = conn.prepareStatement(sql);
